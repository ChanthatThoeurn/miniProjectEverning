...
async function searchMovies() {
  ...
  resultsContainer.innerHTML =
    '<p class="text-white col-span-full">Loading...</p>';

  try {
    const res = await fetch(
      `https://ott-details.p.rapidapi.com/search?title=${encodeURIComponent(query)}`,
      options
    );
    ...
    if (!data.results || data.results.length === 0) {
      resultsContainer.innerHTML =
        '<p class="text-white col-span-full">No results found.</p>';
      return;
    }
    ...
    data.results.forEach((movie) => {
      ...
      card.className =
        "relative group bg-[#1c1c1c] rounded-xl overflow-hidden shadow hover:shadow-lg transition cursor-pointer";
      ...
      card.innerHTML = `
        ...
        <div class="p-2">
          ...
          <p class="text-xs text-gray-400">IMDb: ${
    ...
    resultsContainer.innerHTML =
      '<p class="text-white col-span-full">Error fetching data.</p>';
  }
}
